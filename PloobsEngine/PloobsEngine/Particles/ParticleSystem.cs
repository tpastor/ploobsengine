...
namespace PloobsEngine.Particle3D
{
    ...
    {
        public ParticleSystem()//EngineStuff engine)
        {
            //this.engine = engine;
            LoadContent();
        }
        ...
        {            
            ...
            //vertexDeclaration = new VertexDeclaration(EngineStuff.GraphicsDevice,ParticleVertex.VertexElements);

            // Create a dynamic vertex buffer.
            int size = ParticleVertex.SizeInBytes * particles.Length;

            //vertexBuffer = new DynamicVertexBuffer(EngineStuff.GraphicsDevice, size, BufferUsage.WriteOnly |BufferUsage.Points);
        }
        ...
        {
            //Effect effect = EngineStuff.InternalContentManager.GetAsset<Effect>("particles");
            //Effect effect = EngineStuff.CustomContentManager.GetAsset<Effect>("Effects//particles");
            ...
            //particleEffect = effect.Clone(EngineStuff.GraphicsDevice);
            ...
            //parameters["halfPixel"].SetValue(EngineStuff.HalfPixel);
            parameters["scale"].SetValue(settings.SoftParticleScale);
            parameters["activated"].SetValue(settings.SoftParticleActive ? 1 : 0);            
            //parameters["ScreenResolution"].SetValue(new Vector2(EngineStuff.Viewport.Width,EngineStuff.Viewport.Height));            
            ...
            if (settings.isInternalTexture)
            {
                //texture = EngineStuff.InternalContentManager.GetAsset<Texture2D>(settings.TextureName);
            }
            else
            {
                //texture = EngineStuff.CustomContentManager.GetAsset<Texture2D>(settings.TextureName);
            }
