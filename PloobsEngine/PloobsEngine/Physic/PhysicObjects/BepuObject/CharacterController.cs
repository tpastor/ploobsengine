...
namespace PloobsEngine.Physics.Bepu
{
    public class CharacterController : Updateable, IEndOfTimeStepUpdateable
    {
        ...
        /// <param name="scale">The scale -> scale.x * characterWidth and scale.y * characterHeight  .</param>
        public CharacterController(Vector3 position, float characterHeight, float characterWidth, float mass, float maximumStepHeight, Vector2 scale)
        {
            IsUpdating = false;
        ...
        private bool FindSupport(out BEPUphysics.Entities.Entity supportEntity, out Vector3 supportLocation, out Vector3 supportNormal, out float supportDistance, out Vector3 supportLocationVelocity)
        {
            ...
            BEPUphysics.Entities.Entity hitEntity = null, stepHitEntity = null;
            float distance = float.MaxValue, stepDistance = float.MaxValue;

            foreach (BEPUphysics.Entities.Entity candidate in feetCollisionPairCollector.CollisionInformation.OverlappedEntities)
            {
        ...
        private bool IsStepSafe(float hitDistance)
        {
            ...
            foreach (BEPUphysics.Entities.Entity candidate in headCollisionPairCollector.CollisionInformation.OverlappedEntities)
            {
