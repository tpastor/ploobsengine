...
VertexShaderOutput VShader(VertexShaderInput input)
{
	...
	output.Distance = output.Position.z / output.Position.w;
	output.Distance = 0.5f * (output.Distance + 1 );    
    return output;
}
...
{  
	input.Pos /= ScreenResolution;        
    float myDepth = input.Distance;  
	float sceneDepth = tex2D(Samplerdepth , input.Pos - halfPixel).x;  
	float zFade = saturate(scale * (myDepth - sceneDepth));  
    float4 cor =  tex2D(Sampler, input.TextureCoordinate) * input.Color;
    cor.a *= 1 - zFade;
    return cor;
...
float4 RotatingPixelShader(RotatingPixelShaderInput input) : COLOR0
{
    ...
    float myDepth = input.Distance;  
	float sceneDepth = tex2D(Samplerdepth , input.Pos - halfPixel).x;  
	float zFade = saturate(scale * (myDepth - sceneDepth)) * activated;  
    float4 cor =  tex2D(Sampler, textureCoordinate) * input.Color;
    cor.a *= 1 - zFade;
    return cor;
}
...
technique NonRotatingParticles
{
    pass P0
    {
        ...
        PixelShader = compile ps_4_0 NonRotatingPixelShader();
    }
}
...
technique RotatingParticles
{
    ...
    {
        VertexShader = compile vs_4_0 VShader();
        PixelShader = compile ps_4_0 RotatingPixelShader();
    }
}
